...
MusicChunk musicChunks[MAX_CHUNKS_AMOUNT] = {
    { .time = 1, .len = 90, .key = PIANO_KEY_A, .octave = -16, .on = true },
    { .time = 2, .len = 90, .key = PIANO_KEY_A, .octave = -16, .on = false },
};
...
void loop() {
    ...
    if (isMusicPlaying) {
        ...
        while (musicChunks[chunkIdx].time <= musicTime) {
      ...
      // @TODO: Without delay with rest-time variable
